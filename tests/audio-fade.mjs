...
assert.equal(fadeVolume(0,NaN),0);
assert.equal(fadeVolume(0,10),0);
assert.equal(fadeVolume(.4,10),.25);
assert.equal(fadeVolume(.8,10),.5);
assert.equal(fadeVolume(2,NaN),.5);
...
try {
 ...
 await page.waitForFunction(()=>document.querySelector('audio').volume===0);
 await page.evaluate(()=>{document.querySelector('audio').currentTime=.4;});
 await page.waitForFunction(()=>Math.abs(document.querySelector('audio').volume-.25)<.02);
 ...
 console.log('PASS: 0.8s fade in, 1s fade out, seeking, replay and track changes.');
